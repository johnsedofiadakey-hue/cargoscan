          ...
          return (
            <article key={item.id} className="queue-row review-row">
              ...
              <div className="button-row">
                ...
                <button onClick={() => window.confirm("This will reset the scan status. Continue?") && setPackageStatus(item, "RESCAN_REQUIRED")}>Rescan</button>
              </div>
            </article>
          );
        })}
...
function ShipmentsTab({ data, reload, setMessage, organization }) {
...
function CustomersTab({ data, reload, setMessage }) {
  ...
  async function deleteConsignee(consignee) {
    if (!window.confirm(`Remove ${consignee.name} from your customer list?`)) return;
...
function ContainersTab({ data, reload, setMessage, setError }) {
  ...
  async function removeItem(containerId, itemId) {
    ...
    if (!window.confirm(`Remove ${item?.description || "this package"} from container?`)) return;
  ...
  async function sealContainer(container) {
    if (!window.confirm(`Seal container ${container.number}? This will lock the manifest and queue customer notifications.`)) return;
...
function TeamTab({ data, reload, setMessage }) {
  ...
  async function toggleUser(user) {
    if (!window.confirm(`${user.active ? "Deactivate" : "Reactivate"} ${user.name}?${user.active ? " They won't be able to log in." : ""}`)) return;
...
function BillingTab({ organization, setMessage }) {
  ...
  async function upgrade(plan) {
    ...
    if (!window.confirm(`Upgrade to ${plan} for ${price}/month? Includes higher limits, team workflows, and customer visibility based on plan.`)) return;
...
function SettingsTab({ session, setSession, data, reload, setMessage }) {
...
function Dashboard({ session, setSession }) {
  ...
  const [active, setActiveState] = useState(routeToTab[window.location.pathname] || "shipments");
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");
  const [lastSynced, setLastSynced] = useState(null);
  const [toast, setToast] = useState("");
  const [onboardingDismissed, setOnboardingDismissed] = useState(localStorage.getItem("cs_onboarding_done") === "1");
  const [data, setData] = useState({ shipments: [], items: [], consignees: [], containers: [], users: [], auditLogs: [], apiKeys: [], webhooks: [] });
  ...
  const load = useCallback(async () => {
    try {
      ...
      const adminCalls = isAdmin ? [api("/users"), api("/audit-logs?page=1&pageSize=50"), api("/keys"), api("/webhooks")] : [Promise.resolve([]), Promise.resolve([]), Promise.resolve([]), Promise.resolve([])];
      ...
      setData((previous) => {
        ...
        return {
          ...
          items: nextItems,
          consignees: listData(consignees),
          containers: listData(containers),
          users: listData(users),
          auditLogs: listData(auditLogs),
          apiKeys: listData(apiKeys),
  ...
  useEffect(() => {
    function onPopState() {
      setActiveState(routeToTab[window.location.pathname] || "shipments");
    }
      ...
      <section className="portal-main">
        <header className="portal-topbar">
          ...
          <div className="user-pill">
            <span className="plan-chip">{session.organization?.plan || "TRIAL"}{planDays !== null ? ` — ${planDays} days left` : ""}</span>
        ...
        {active === "shipments" && <ShipmentsTab data={data} reload={load} setMessage={setMessage} organization={session.organization} />}
        ...
        {active === "settings" && isAdmin && <SettingsTab session={session} setSession={setSession} data={data} reload={load} setMessage={setMessage} />}
