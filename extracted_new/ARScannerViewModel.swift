...
enum ScanPhase: Equatable {
    ...
    case scanning           // actively collecting frames
...
enum GuidanceInstruction: Equatable {
    ...
    case capturing          // all gates green, recording
    case done
    ...
    var message: String {
        ...
        case .capturing:        return "Perfect — hold still, scanning…"
    ...
    var isGood:    Bool { self == .capturing || self == .done }
    var isWarning: Bool {
        switch self {
...
final class ARScannerViewModel: NSObject, ObservableObject, ARSessionDelegate {
    ...
    private func updateGuidance(_ frame: ARFrame) {
        ...
        // Promote/demote readyToScan
        if instr.isGood   && phase == .positioning  { phase = .readyToScan }
    ...
    private func runMeasurementFrame(_ frame: ARFrame) {
        // Only capture when guidance is green
        guard guidance == .capturing,
        ...
        // ── 8. Project object points onto top plane ───────────────────────────
        let pts2D: [simd_float2] = abovePts.map {
            let proj = topPlane.project($0)
            return simd_float2(simd_dot(proj, right),
                               simd_dot(proj, fwd))
        }
        guard pts2D.count >= 8 else { return }

        // ── 9. Convex hull + minimum bounding rectangle ───────────────────────
        let hull = ConvexHull2D.compute(pts2D)
        guard hull.count >= 3 else { return }
        let (longM, shortM) = ConvexHull2D.minBoundingRect(hull: hull)

        let lengthCm = longM  * 100.0
        let widthCm  = shortM * 100.0
        guard lengthCm >= minimumDimCm,
              widthCm  >= minimumDimCm else { return }

        // ── 10. Confidence ────────────────────────────────────────────────────
        let density     = min(Float(topSurface.count) / 80.0, 1.0)
        let aspect      = min(shortM / max(longM, 0.001), 1.0)
        let aspectScore: Float = aspect > 0.1 ? 1.0 : aspect * 10.0
        let conf        = min(density * 0.6 + aspectScore * 0.4, Float(0.99))
