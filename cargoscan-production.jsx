...
const G = () => (
  <style>{`
    ...
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
    html,body{background:${C.bg};color:${C.txt};font-family:${C.font};min-height:100vh;overflow-x:hidden;-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility;}
    ::-webkit-scrollbar{width:4px;height:4px;}
    ::-webkit-scrollbar-track{background:transparent;}
    ::-webkit-scrollbar-thumb{background:${C.bd2};border-radius:4px;}
    ...
    .asl{animation:SL .35s ease both;}
...
const PLATFORM = {
  ...
  orgs: {
    "stormglide":{
      id:"o1", name:"Stormglide Logistics", slug:"stormglide",
      country:"Ghana", city:"Accra", plan:"BUSINESS", cbmRate:85,
      trial:null, createdAt:"2026-01-08",
      usage:{ ships:18, items:342, users:4 }, limits:{ ships:200, items:99999, users:10 },
      sub:"SUB_stm_xxx",
    },
    "fastfreight":{
      id:"o2", name:"FastFreight GH", slug:"fastfreight",
      country:"Ghana", city:"Tema", plan:"TRIAL", cbmRate:85,
      trial:7, createdAt:"2026-02-22",
      usage:{ ships:3, items:41, users:2 }, limits:{ ships:5, items:50, users:2 },
      sub:null,
    },
    "guangzhou-premier":{
      id:"o3", name:"Guangzhou Premier", slug:"guangzhou-premier",
      country:"China", city:"Guangzhou", plan:"ENTERPRISE", cbmRate:90,
      trial:null, createdAt:"2025-12-01",
      usage:{ ships:87, items:1240, users:12 }, limits:{ ships:99999, items:99999, users:99999 },
      sub:"SUB_gp_yyy",
    },
  },

  users: [
    { id:"u1",org:"stormglide",   name:"John Mensah",   email:"john@stormglide.com",  pass:"Admin1234!",   role:"ADMIN",      active:true,  seen:"2 min ago" },
    { id:"u2",org:"stormglide",   name:"Ama Owusu",     email:"ama@stormglide.com",   pass:"Ama12345!",    role:"SUPERVISOR", active:true,  seen:"1 hr ago" },
    { id:"u3",org:"stormglide",   name:"James Asante",  email:"james@stormglide.com", pass:"Ops12345!",    role:"OPERATOR",   active:true,  seen:"4 hr ago" },
    { id:"u4",org:"stormglide",   name:"Kofi Darko",    email:"kofi@stormglide.com",  pass:"Kofi1234!",    role:"OPERATOR",   active:false, seen:"12d ago" },
    { id:"u5",org:"fastfreight",  name:"Eric Boateng",  email:"eric@fastfreight.com", pass:"Eric1234!",    role:"ADMIN",      active:true,  seen:"Just now" },
    { id:"u6",org:"guangzhou-premier", name:"Wei Zhang",email:"wei@guangzhou.com",    pass:"WeiAdmin99!",  role:"ADMIN",      active:true,  seen:"5 min ago" },
  ],
  ...
  pricing: {
    trial_days:14, trial_users:2, trial_ships:5, trial_items:50,
  ...
  stats: {
    mrr:4120, arr:49440, orgs:47, activeOrgs:42, churn:3.2, ltv:2133, conv:38.2,
    items30d:8420, wa30d:6102, waDelivery:94.1, openDisputes:8,
    services:[
  ...
  shipments:[
    { id:"s1",code:"SHP-2026-001",from:"Guangzhou",to:"Tema", cbm:34.2,cap:55.8,items:14,status:"IN_TRANSIT",color:C.blue },
    { id:"s2",code:"SHP-2026-002",from:"Shenzhen", to:"Accra",cbm:18.5,cap:55.8,items:8, status:"RECEIVING", color:C.amber },
    { id:"s3",code:"SHP-2026-003",from:"Guangzhou",to:"Tema", cbm:55.8,cap:55.8,items:22,status:"DELIVERED", color:C.green },
    { id:"s4",code:"SHP-2026-004",from:"Guangzhou",to:"Accra",cbm:12.1,cap:28,  items:6, status:"PENDING",   color:C.purple },
  ],
};
...
function Toast() {
  ...
  return (
    <div style={{position:"fixed",bottom:24,right:24,zIndex:9999,animation:"FU .3s ease",
...
const Logo = ({sz=20,showText=true}) => (
  <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
    <div style={{width:sz+10,height:sz+10,background:`linear-gradient(135deg,${C.blue},${C.purple})`,
...
const Spin = ({sz=14,col="#fff"}) => (
  <div style={{width:sz,height:sz,border:`2px solid ${col}25`,borderTopColor:col,
...
function Btn({label,icon,onClick,v="primary",sz="md",disabled,loading,full,style={}}) {
  ...
  return (
    <button onClick={disabled||loading?undefined:onClick} style={{
      ...
      padding:pad,fontSize:fs,fontWeight:700,letterSpacing:"-.01em",
    ...
    <div style={{marginBottom:14}}>
      ...
      <div style={{position:"relative"}}>
        ...
        <input type={type} value={value} onChange={e=>onChange?.(e.target.value)}
          ...
          style={{width:"100%",background:readOnly?C.dim:C.s2,border:`1px solid ${err?C.red:focus?C.blue:C.bd}`,
...
const Chip = ({label,color=C.blue,dot}) => (
  <span style={{fontFamily:C.mono,fontSize:9,fontWeight:600,letterSpacing:".1em",textTransform:"uppercase",
  ...
  return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 0",borderBottom:`1px solid ${C.bd}`}}>
      ...
      <div onClick={()=>onChange(!on)} style={{width:40,height:22,borderRadius:11,
        ...
        <div style={{position:"absolute",top:3,left:on?19:3,width:16,height:16,borderRadius:"50%",
...
function Modal({title,children,onClose,width=500}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:1000,
      ...
      <div className="azi" style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:14,
...
function LoginScreen({onSuccess,onSignup}) {
  ...
  const attempt = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (email===PLATFORM._sa.email && pass===PLATFORM._sa.pass) {
        onSuccess({ type:"superadmin", user:{ name:PLATFORM._sa.name, email } });
        return;
      }
      // Org users
      const u = PLATFORM.users.find(x=>x.email===email && x.pass===pass);
      if (u) {
        if (!u.active) { setErr("Your account has been deactivated. Contact your organisation admin."); setLoad(false); return; }
        const org = PLATFORM.orgs[u.org];
  ...
  return (
    <div style={{minHeight:"100vh",display:"grid",gridTemplateColumns:"420px 1fr"}}>
      {/* LEFT — brand panel */}
      <div style={{background:C.s1,borderRight:`1px solid ${C.bd}`,display:"flex",
        flexDirection:"column",justifyContent:"space-between",padding:"44px 44px 32px",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:"-5%",right:"-30%",width:400,height:400,
          background:`radial-gradient(circle,${C.blueGlow} 0%,transparent 70%)`,pointerEvents:"none"}}/>
        <div style={{position:"absolute",bottom:"10%",left:"-20%",width:300,height:300,
        ...
        <div className="afu">
          ...
          <h1 style={{fontSize:40,fontWeight:800,lineHeight:.94,letterSpacing:"-.04em",marginBottom:18}}>
            ...
            <span style={{background:`linear-gradient(90deg,${C.blue},${C.purple})`,
              WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Zero disputes.</span>
          </h1>
      ...
      {/* RIGHT — form */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:40,background:C.bg}}>
        <div className="afu" style={{width:"100%",maxWidth:380}}>
          <h2 style={{fontSize:27,fontWeight:800,letterSpacing:"-.03em",marginBottom:4}}>Welcome back</h2>
          <p style={{color:C.mid,fontSize:13,marginBottom:28}}>Sign in to your workspace</p>

          {err&&<div style={{background:`${C.red}12`,border:`1px solid ${C.red}30`,borderRadius:9,
          ...
          <div style={{marginBottom:14}}>
            ...
            <div style={{position:"relative"}}>
              <input id="cs-pw" type={show?"text":"password"} value={pass} onChange={e=>setPass(e.target.value)}
                ...
                style={{width:"100%",background:C.s2,border:`1px solid ${C.bd}`,borderRadius:9,
                  color:C.txt,fontSize:13,fontWeight:500,padding:"10px 48px 10px 12px",outline:"none"}}/>
              <span onClick={()=>setShow(p=>!p)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",
...
function SignupScreen({onDone,onLogin}) {
  ...
  const next = () => {
    ...
    if(step===2&&v2()) {
      setLoad(true);
      setTimeout(()=>{
        setCreated({ name:f.name,email:f.email,password:f.pass,company:f.company,slug,plan:"TRIAL",role:"ADMIN" });
  ...
  if(step===3&&created) return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg,padding:20}}>
      <div className="afu" style={{maxWidth:520,width:"100%",textAlign:"center"}}>
        <div style={{width:72,height:72,background:`linear-gradient(135deg,${C.green},${C.greenD})`,borderRadius:20,
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 22px"}}>✓</div>
        <h1 style={{fontSize:28,fontWeight:800,letterSpacing:"-.03em",marginBottom:8}}>Account created!</h1>
        <p style={{color:C.mid,fontSize:13,marginBottom:6}}><strong style={{color:C.txt}}>{created.company}</strong> is live on CargoScan.</p>
        <div style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:12,padding:20,margin:"20px 0",textAlign:"left"}}>
          ...
          {[["Portal URL",`${created.slug}.cargoscan.app`,C.blue],["Email",created.email,C.txt],["Password",created.password,C.txt],["Plan","TRIAL — 14 days free",C.amber],["Role","ADMIN",C.green]].map(([k,v,col])=>(
        ...
        <Btn label="Enter Your Dashboard →" v="primary" full sz="lg" onClick={onDone}/>
        <p style={{fontSize:11,color:C.mid,marginTop:12}}>Trial ends in 14 days. Upgrade anytime from $29/month.</p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",background:C.bg}}>
      {/* Steps sidebar */}
      <div style={{width:340,background:C.s1,borderRight:`1px solid ${C.bd}`,padding:"40px 32px",display:"flex",flexDirection:"column",flexShrink:0}}>
        <Logo sz={20}/>
        <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",paddingTop:48}}>
          {[["Create account","Name, email & password"],["Set up company","Workspace & subdomain"],["You're in!","14-day trial begins"]].map(([s,d],i)=>(
            <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start",marginBottom:24}}>
              <div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,transition:"all .3s",
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:40}}>
        <div className="afu" style={{maxWidth:420,width:"100%"}}>
          ...
          {step===2&&<>
            ...
            {f.company&&<div style={{background:`${C.blue}12`,border:`1px solid ${C.blue}25`,borderRadius:8,padding:"9px 14px",
...
function SuperAdmin({onLogout}) {
  ...
  const [flags,setFlags]     = useState(()=>({...PLATFORM.flags}));
  const [pricing,setPricing] = useState(()=>({...PLATFORM.pricing}));
  const [orgs,setOrgs]       = useState(()=>Object.values(PLATFORM.orgs));
  const [psMode,setPsMode]   = useState("test");
  const [overrideTarget,setOvT]= useState(null);
  const [confirm,setConfirm]   = useState(null);

  const saveFlags = () => { notify("Feature flags saved — active immediately","ok"); };

  const applyOverride = (orgId,plan,days,reason) => {
    setOrgs(p=>p.map(o=>o.id===orgId?{...o,plan}:o));
    notify(`Plan override applied: ${plan} for ${days} days`,"ok");
    setOvT(null);
  };
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",background:C.bg}}>
      {/* TOP */}
      <div style={{background:C.s1,borderBottom:`1px solid ${C.bd}`,padding:"0 24px",
      ...
      <div style={{background:C.s1,borderBottom:`1px solid ${C.bd}`,display:"flex",padding:"0 20px",overflowX:"auto",flexShrink:0}}>
        {TABS.map(t=>(
          <div key={t.id} onClick={()=>setTab(t.id)} style={{padding:"11px 14px",cursor:"pointer",
      ...
      <div style={{flex:1,padding:24,overflowY:"auto"}}>
        ...
        {overrideTarget&&<Modal title={`Override Plan — ${overrideTarget.name}`} onClose={()=>setOvT(null)}>
          <OverrideForm org={overrideTarget} allOrgs={orgs} onSave={applyOverride} onCancel={()=>setOvT(null)}/>
        </Modal>}
        ...
        {tab==="overview"&&<div className="afu">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
            <Stat label="Monthly Recurring Revenue" value={`$${PLATFORM.stats.mrr.toLocaleString()}`} sub="↑ 18% vs last month" color={C.blue}/>
            <Stat label="Active Organisations" value={PLATFORM.stats.activeOrgs} sub="6 new this month" color={C.green}/>
            <Stat label="Items Scanned (30d)" value={PLATFORM.stats.items30d.toLocaleString()} sub="↑ 24%" color={C.purple}/>
            <Stat label="WhatsApp Delivery Rate" value={`${PLATFORM.stats.waDelivery}%`} sub={`${PLATFORM.stats.wa30d.toLocaleString()} messages sent`} color={C.cyan}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Card style={{padding:20}}>
              <div style={{fontWeight:700,fontSize:13,marginBottom:14}}>Plan Distribution</div>
              {[["Enterprise",3,C.purple],["Business",18,C.green],["Starter",14,C.blue],["Trial",12,C.amber]].map(([n,c,col])=>(
                <div key={n} style={{marginBottom:10}}>
                  ...
                  <div style={{height:5,background:C.bd,borderRadius:3}}>
                    <div style={{height:"100%",width:`${(c/47)*100}%`,background:col,borderRadius:3}}/>
                  </div>
                </div>
              ))}
            </Card>
            <Card style={{padding:20}}>
              ...
              {[["ARR (projected)",`$${PLATFORM.stats.arr.toLocaleString()}`,C.blue],
                ["Avg LTV",`$${PLATFORM.stats.ltv.toLocaleString()}`,C.green],
                ["Trial → Paid conv.",`${PLATFORM.stats.conv}%`,C.cyan],
                ["Churn rate",`${PLATFORM.stats.churn}%`,C.amber],
                ["Open disputes",PLATFORM.stats.openDisputes,C.red],
                ["Total orgs",PLATFORM.stats.orgs,C.txt]].map(([l,v,col])=>(
        ...
        {tab==="orgs"&&<div className="afu">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
            ...
            <Chip label={`${orgs.length} total`} color={C.mid}/>
          </div>
          <Card>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr>{["Company","Plan","Users","Shipments","Since","MRR","Status",""].map(h=>(
                <th key={h} style={{fontFamily:C.mono,fontSize:9,fontWeight:600,letterSpacing:".12em",color:C.mid,
                  textTransform:"uppercase",padding:"10px 16px",textAlign:"left",borderBottom:`1px solid ${C.bd}`}}>{h}</th>
              ))}</tr></thead>
              <tbody>{orgs.map(o=>(
                <tr key={o.id} style={{borderBottom:`1px solid ${C.bd}`}}>
                  ...
                  <td style={{padding:"11px 16px"}}><Chip label={o.plan} color={planColor(o.plan)}/></td>
                  <td style={{padding:"11px 16px",fontFamily:C.mono,fontSize:12}}>{o.usage.users}</td>
                  <td style={{padding:"11px 16px",fontFamily:C.mono,fontSize:12}}>{o.usage.ships}</td>
                  <td style={{padding:"11px 16px",fontSize:11,color:C.mid}}>{o.createdAt}</td>
                  <td style={{padding:"11px 16px",fontFamily:C.mono,fontSize:12,color:C.green}}>${planMrr(o.plan)}/mo</td>
                  <td style={{padding:"11px 16px"}}>
                    <Chip label={o.plan==="TRIAL"?"TRIAL":"ACTIVE"} color={o.plan==="TRIAL"?C.amber:C.green} dot={o.plan!=="TRIAL"?"pulse":undefined}/>
                  </td>
                  <td style={{padding:"11px 16px"}}><Btn label="Override" v="ghost" sz="sm" onClick={()=>setOvT(o)}/></td>
                </tr>
              ))}</tbody>
            </table>
          </Card>
...
PAYSTACK_ENTERPRISE_PLAN_CODE=${pricing.ps_enterprise||"PLN_replace"}`}
                ...
                <div style={{display:"flex",gap:6}}>
                  {["test","live"].map(m=>(
                    <div key={m} onClick={()=>{setPsMode(m);if(m==="live")notify("⚠ LIVE mode — real charges","err");else notify("Test mode active","ok");}}
                      style={{flex:1,textAlign:"center",padding:"9px 0",borderRadius:8,cursor:"pointer",transition:"all .15s",
        ...
        {tab==="override"&&<div className="afu">
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            ...
            <Card style={{padding:20}}>
              <div style={{fontWeight:700,fontSize:13,marginBottom:14}}>Recent Overrides</div>
              {[["Accra Cargo → BUSINESS","admin","2 hours ago",C.green],["Lagos Freight → STARTER","admin","1 day ago",C.blue],["Test Org → ENTERPRISE","admin","3 days ago",C.purple]].map(([a,by,when,c])=>(
                <div key={a} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:`1px solid ${C.bd}`}}>
                  <div><div style={{fontSize:12,fontWeight:600}}>{a}</div><div style={{fontSize:11,color:C.mid,marginTop:1}}>by {by} · {when}</div></div>
                  <Chip label="OVERRIDE" color={c}/>
                </div>
              ))}
            </Card>
          </div>
        ...
        {tab==="logs"&&<div className="afu">
          ...
          <Card>
            <div style={{fontFamily:C.mono,fontSize:11,padding:16,height:500,overflowY:"auto",lineHeight:2}}>
              {[["POST /api/auth/login","200 · 82ms · john@stormglide.com",C.green],["POST /api/items","201 · CBM:0.224 · SHP-2026-001",C.blue],["POST /api/billing/paystack/webhook","200 · charge.success · BUSINESS activated",C.cyan],["WARN /api/items","429 · TRIAL LIMIT · fastfreight items:50/50",C.amber],["POST /api/items/abc/verify","200 · autoApproved · diff:1.2%",C.green],["POST /api/disputes","DSP-019 opened · mismatch 14.2%",C.red],["GET /health","200 · 3ms",C.green],["POST /api/admin/override","BUSINESS override applied · stormglide",C.purple]].map(([path,detail,c],i)=>(
                <div key={i} style={{display:"flex",gap:16,alignItems:"flex-start"}}>
                  <span style={{color:c,minWidth:8,fontWeight:700}}>→</span>
                  <span style={{color:C.txt,minWidth:260}}>{path}</span>
                  <span style={{color:C.mid}}>{detail}</span>
                </div>
              ))}
            </div>
          </Card>
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      ...
      <Card style={{padding:20}}>
        ...
        <div style={{background:C.bg,border:`1px solid ${C.bd}`,borderRadius:10,padding:16,
...
function OrgApp({user,org,onLogout}) {
  ...
  const [orgUsers,setOrgUsers]= useState(()=>PLATFORM.users.filter(u=>u.org===org.slug));
  ...
  const TABS = [
    ...
    {id:"team",      icon:"👥", label:"Team",         show:isAdmin},
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",background:C.bg}}>
      {/* TRIAL BANNER */}
      {trial!==null&&(
        <div style={{background:urgent?`${C.red}14`:`${C.amber}10`,borderBottom:`1px solid ${urgent?C.red:C.amber}30`,
      ...
      {/* TOP NAV */}
      <div style={{background:C.s1,borderBottom:`1px solid ${C.bd}`,height:52,padding:"0 20px",
        display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <Logo sz={16}/>
          <div style={{width:1,height:20,background:C.bd}}/>
          <div>
            <div style={{fontWeight:700,fontSize:13,lineHeight:1}}>{org2.name}</div>
            <div style={{fontFamily:C.mono,fontSize:9,color:C.blue,marginTop:2,letterSpacing:".09em"}}>{org2.slug}.cargoscan.app</div>
          </div>
          <Chip label={org2.plan} color={planColor(org2.plan)}/>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          {isAdmin&&org2.plan==="TRIAL"&&<Btn label="⚡ Upgrade" sz="sm" onClick={()=>setUpg(true)}/>}
          <div style={{display:"flex",alignItems:"center",gap:8,background:C.s2,border:`1px solid ${C.bd}`,borderRadius:8,padding:"5px 12px"}}>
            <Avatar name={user.name} sz={24} color={user.role==="ADMIN"?C.blue:user.role==="SUPERVISOR"?C.purple:C.green}/>
            <div>
              <div style={{fontSize:12,fontWeight:700,lineHeight:1}}>{user.name}</div>
              <div style={{fontSize:9,color:C.mid,fontWeight:600,letterSpacing:".08em",textTransform:"uppercase",marginTop:1}}>{user.role}</div>
            </div>
          </div>
          <Btn label="Sign Out" v="ghost" sz="sm" onClick={onLogout}/>
        </div>
      ...
      {/* TAB BAR */}
      <div style={{background:C.s1,borderBottom:`1px solid ${C.bd}`,display:"flex",padding:"0 16px",overflowX:"auto",flexShrink:0}}>
        {TABS.map(t=>(
          <div key={t.id} onClick={()=>setTab(t.id)} style={{padding:"11px 14px",cursor:"pointer",whiteSpace:"nowrap",
            fontSize:12,fontWeight:600,display:"flex",alignItems:"center",gap:5,transition:"color .15s",
            color:tab===t.id?C.txt:C.mid,
            borderBottom:`2px solid ${tab===t.id?(t.accent?C.green:C.blue):"transparent"}`,
            background:t.accent&&tab!==t.id?`${C.green}06`:"transparent"}}>
            {t.icon} {t.label}
            {t.accent&&<Chip label="iOS" color={C.green}/>}
          </div>
        ))}
      </div>
      ...
      <div style={{flex:1,padding:22,overflowY:"auto"}}>
        ...
        {tab==="team"&&isAdmin&&<TeamTab
          org={org2} user={user} orgUsers={orgUsers}
          setOrgUsers={setOrgUsers}
          onUpgrade={()=>setUpg(true)}
          onInvite={()=>setInv(true)}
        />}
  ...
  return (
    <div className="afu">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:18}}>
        <Stat label="Items Scanned (30d)" value="342" sub="↑ 24% vs last month" color={C.blue}/>
        <Stat label="Total CBM (30d)"    value="41.8" sub="↑ 18% vs last month" color={C.green}/>
        <Stat label="Active Shipments"   value="3"    sub="12 total this month" color={C.purple}/>
        <Stat label="Open Disputes"      value="2"    sub="vs 5 last month ↓" color={C.amber}/>
      </div>
      ...
      <Card>
        <div style={{padding:"14px 18px",borderBottom:`1px solid ${C.bd}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <Btn label="+ New Shipment" sz="sm" v="ghost" onClick={()=>notify("Opens create shipment form","info")}/>
        </div>
        {PLATFORM.shipments.map(s=>(
...
function ShipmentsTab({org,user}) {
  ...
  const filtered = filter==="ALL"?PLATFORM.shipments:PLATFORM.shipments.filter(s=>s.status===filter);

  return (
    <div className="afu">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        <h2 style={{fontSize:20,fontWeight:800,letterSpacing:"-.02em"}}>Shipments</h2>
        <Btn label="+ Create Shipment" onClick={()=>notify("Opens create shipment modal","info")}/>
      </div>
      <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
        {statuses.map(s=>(
          <div key={s} onClick={()=>setFilter(s)} style={{padding:"5px 14px",borderRadius:20,cursor:"pointer",fontSize:11,fontWeight:600,
        ...
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>{filtered.map(s=>(
            <tr key={s.id} style={{borderBottom:`1px solid ${C.bd}`}}>
              <td style={{padding:"11px 16px",fontFamily:C.mono,fontSize:11,color:C.blue,fontWeight:700}}>{s.code}</td>
              <td style={{padding:"11px 16px",fontSize:12,color:C.mid}}>{s.from} → {s.to}</td>
              <td style={{padding:"11px 16px",fontFamily:C.mono,fontSize:12,fontWeight:700,color:s.color}}>{s.cbm}</td>
              ...
              <td style={{padding:"11px 16px",fontFamily:C.mono,fontSize:12}}>{s.items}</td>
              <td style={{padding:"11px 16px"}}><Chip label={s.status} color={s.color}/></td>
              <td style={{padding:"11px 16px"}}>
                <div style={{display:"flex",gap:6}}>
...
function ScanTab({org}) {
  ...
  const cbm = dims.l&&dims.w&&dims.h ? ((parseFloat(dims.l)*parseFloat(dims.w)*parseFloat(dims.h))/1000000).toFixed(4) : null;
  const cost = cbm ? (parseFloat(cbm)*org.cbmRate).toFixed(2) : null;

  return (
    <div className="afu">
      <h2 style={{fontSize:20,fontWeight:800,letterSpacing:"-.02em",marginBottom:4}}>Scan Item</h2>
      <p style={{fontSize:12,color:C.mid,marginBottom:20}}>LiDAR scanning runs in the iOS app. Use the manual calculator below for testing or non-LiDAR devices.</p>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        <Card style={{padding:22}}>
          <div style={{fontWeight:700,fontSize:13,marginBottom:4}}>iPhone LiDAR Scanner</div>
          <p style={{fontSize:12,color:C.mid,marginBottom:18}}>Download CargoScan from TestFlight on your iPhone 12 Pro or newer. Sign in with your account and start scanning.</p>
          <div style={{background:C.dim,border:`1px dashed ${C.bd2}`,borderRadius:10,padding:28,textAlign:"center",marginBottom:16}}>
            <div style={{fontSize:40,marginBottom:10}}>📱</div>
            <div style={{fontSize:13,fontWeight:600,marginBottom:4}}>Open on iPhone 12 Pro+</div>
            <div style={{fontSize:11,color:C.mid}}>ARKit LiDAR · ±1cm accuracy · 30,000 depth points/sec</div>
          </div>
          <Btn label="Download via TestFlight" v="ghost" full onClick={()=>notify("TestFlight link opens in your browser","info")}/>
        </Card>
        <Card style={{padding:22}}>
          <div style={{fontWeight:700,fontSize:13,marginBottom:14}}>Manual CBM Calculator</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:14}}>
            {["l","w","h"].map(k=>(
              <div key={k}>
                ...
                <input type="number" value={dims[k]} onChange={e=>setDims(p=>({...p,[k]:e.target.value}))} placeholder="0"
          ...
          {cbm&&(
            <div style={{background:C.bg,border:`1px solid ${C.bd}`,borderRadius:10,padding:16,marginBottom:14}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div><div style={{fontSize:10,color:C.mid,letterSpacing:".09em",textTransform:"uppercase",marginBottom:4}}>CBM</div>
                  <div style={{fontFamily:C.mono,fontSize:24,fontWeight:700,color:C.blue}}>{cbm}</div></div>
                <div><div style={{fontSize:10,color:C.mid,letterSpacing:".09em",textTransform:"uppercase",marginBottom:4}}>Shipping cost</div>
                  <div style={{fontFamily:C.mono,fontSize:24,fontWeight:700,color:C.green}}>${cost}</div></div>
              </div>
              <div style={{fontSize:10,color:C.mid,marginTop:8}}>Rate: ${org.cbmRate}/CBM · Formula: L×W×H÷1,000,000</div>
            </div>
          )}
          <Btn label="Save Item + Notify Customer" v="green" full disabled={!cbm} onClick={()=>notify("Item saved and WhatsApp notification sent ✓","ok")}/>
        </Card>
      </div>
    </div>
  );
...
function DisputesTab() {
  ...
  const disputes = [
    {id:"DSP-019",code:"ITM-047",customer:"Abena Mensah",origin:"0.224",dest:"0.261",diff:"16.5%",status:"OPEN",priority:"HIGH"},
    {id:"DSP-018",code:"ITM-033",customer:"Kwame Asare",origin:"0.180",dest:"0.195",diff:"8.3%",status:"PENDING",priority:"MEDIUM"},
    {id:"DSP-015",code:"ITM-021",customer:"Ama Boateng",origin:"0.442",dest:"0.448",diff:"1.4%",status:"RESOLVED",priority:"LOW"},
  ];
        ...
        <Card style={{padding:22}}>
          ...
          {sel.status!=="RESOLVED"&&(
            <div>
              ...
              <Btn label="Mark as Resolved" v="green" onClick={()=>{setSel(p=>({...p,status:"RESOLVED"}));notify(`${sel.id} resolved ✓`,"ok");}}/>
            </div>
          )}
        </Card>
      ):(
        <Card>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr>{["Dispute ID","Item Code","Customer","Origin CBM","Dest CBM","Diff","Status","Priority",""].map(h=>(
              <th key={h} style={{fontFamily:C.mono,fontSize:9,fontWeight:600,letterSpacing:".1em",color:C.mid,textTransform:"uppercase",padding:"10px 14px",textAlign:"left",borderBottom:`1px solid ${C.bd}`}}>{h}</th>
            ))}</tr></thead>
            <tbody>{disputes.map(d=>(
              <tr key={d.id} style={{borderBottom:`1px solid ${C.bd}`}}>
                <td style={{padding:"10px 14px",fontFamily:C.mono,fontSize:11,color:C.blue,fontWeight:700}}>{d.id}</td>
                <td style={{padding:"10px 14px",fontFamily:C.mono,fontSize:11}}>{d.code}</td>
                <td style={{padding:"10px 14px",fontSize:12}}>{d.customer}</td>
                <td style={{padding:"10px 14px",fontFamily:C.mono,fontSize:12,color:C.blue}}>{d.origin}</td>
                <td style={{padding:"10px 14px",fontFamily:C.mono,fontSize:12,color:C.amber}}>{d.dest}</td>
                <td style={{padding:"10px 14px",fontFamily:C.mono,fontSize:12,fontWeight:700,color:d.status==="RESOLVED"?C.green:C.red}}>{d.diff}</td>
                <td style={{padding:"10px 14px"}}><Chip label={d.status} color={statusColor(d.status)}/></td>
                <td style={{padding:"10px 14px"}}><Chip label={d.priority} color={d.priority==="HIGH"?C.red:d.priority==="MEDIUM"?C.amber:C.mid}/></td>
                <td style={{padding:"10px 14px"}}><Btn label="Review" v="ghost" sz="sm" onClick={()=>setSel(d)}/></td>
              </tr>
            ))}</tbody>
          </table>
        </Card>
  ...
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:20}}>
        {plans.map(p=>(
          <div key={p.key} onClick={()=>setSel(p.key)} style={{background:sel===p.key?`${p.col}14`:C.s2,
...
export default function CargoScan() {
  ...
  return (
    <>
      ...
      {screen==="signup" && <SignupScreen onDone={()=>{setSession({type:"org",user:PLATFORM.users[0],org:Object.values(PLATFORM.orgs)[1]});setScreen("app");}} onLogin={()=>setScreen("login")}/>}
      {screen==="app" && session?.type==="superadmin" && <SuperAdmin onLogout={onLogout}/>}
